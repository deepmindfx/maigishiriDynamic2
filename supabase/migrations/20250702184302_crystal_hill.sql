...
DROP TRIGGER IF EXISTS update_ticket_on_new_message ON ticket_messages;
...
-- Create policies for support_tickets with checks to avoid duplicates
DO $$
BEGIN
...
-- Create policies for ticket_messages with checks to avoid duplicates
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    ...
    AND policyname = 'Users can view messages for their own tickets'
  ) THEN
    EXECUTE $policy$
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'ticket_messages' 
    AND policyname = 'Users can add messages to their own tickets'
  ) THEN
    EXECUTE $policy$
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'ticket_messages' 
    AND policyname = 'Admins can add messages to any ticket'
  ) THEN
    EXECUTE $policy$
